...
public class EntityStore<TDbContext, TEntity>(
    IDbContextFactory<TDbContext> dbContextFactory,
    IDbExceptionHandler<TDbContext> exceptionHandler)
    : Store<TDbContext, TEntity>(dbContextFactory, exceptionHandler)
    where TDbContext : DbContext where TEntity : Entity, new()
{
    /// <summary>
    ///     Saves the entity.
    /// </summary>
    ...
    {
        await SaveAsync(entity, null, cancellationToken);
    }
    ...
    /// <param name="cancellationToken">The cancellation token.</param>
    public async Task SaveAsync(TEntity entity, Func<TDbContext, TEntity, CancellationToken, ValueTask>? onSaving,
        CancellationToken cancellationToken = default)
    {
        await SaveAsync(entity, x => x.Id, onSaving, cancellationToken);
    }
    ...
    {
        await SaveManyAsync(entities, default, cancellationToken);
    }
    ...
    public async Task SaveManyAsync(IEnumerable<TEntity> entities,
        ...
        CancellationToken cancellationToken = default)
    {
        await SaveManyAsync(entities, x => x.Id, onSaving, cancellationToken);
    }
}